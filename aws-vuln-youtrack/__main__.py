...
app_cluster = aws.ecs.Cluster("youtrack-cluster")

# Creating a VPC and a public subnet
app_vpc = aws.ec2.Vpc("youtrack-vpc",
    cidr_block="172.31.0.0/16",
    enable_dns_hostnames=True)

app_vpc_subnet = aws.ec2.Subnet("youtrack-vpc-subnet",
    cidr_block="172.31.32.0/20",
    vpc_id=app_vpc.id)

# Creating a gateway to the web for the VPC and a route
app_gateway = aws.ec2.InternetGateway("youtrack-gateway",
    vpc_id=app_vpc.id)

app_routetable = aws.ec2.RouteTable("youtrack-routetable",
    routes=[
...
app_routetable_association = aws.ec2.MainRouteTableAssociation("youtrack_routetable_association",
...
ro_policy = aws.iam.Policy("roPolicy", 
...
# Attaching s3 ro to the ro task role
task_policy_attachment = aws.iam.RolePolicyAttachment("app-ro-access-policy", role=app_task_role.name,
...
youtrack_balancer = aws.lb.LoadBalancer("youtrack-balancer",
...
youtrack_targetgroup = aws.lb.TargetGroup("youtrack-targetgroup",
    port=80,
    protocol="TCP",
    target_type="ip",
...
youtrack_listener = aws.lb.Listener("youtrack-listener",
...
youtrack_image = docker.Image("youtrack-dockerimage",
...
youtrack_task_definition = aws.ecs.TaskDefinition("youtrack-task-definition",
...
youtrack_service = aws.ecs.Service("youtrack-service",
    ...
    load_balancers=[aws.ecs.ServiceLoadBalancerArgs(
		target_group_arn=youtrack_targetgroup.arn,
		container_name="youtrack-container",
		container_port=8080,
	)],
...
bucket = aws.s3.Bucket("CTF-d276243c33a98f677e1c679f8b1353b2",
    acl="private",
    tags={
        "Environment": "Dev",
    })
...
pulumi.export("app-url", youtrack_balancer.dns_name)